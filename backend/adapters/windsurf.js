 ...
 *
 * Windsurf uses Cmd+L to focus the Cascade chat panel.
 * App identifier: "Windsurf"
 */
...
class WindsurfAdapter extends BaseAdapter {
  ...
  getChatFocusShortcut() {
    // Cmd+L focuses the Cascade chat in Windsurf
  ...
  async isRunning() {
    return new Promise((resolve) => {
      exec(
        `osascript -e 'tell application "System Events" to (name of processes) contains "Windsurf"'`,
        (err, stdout) => {
...
tell application "System Events"
  try
    set wsProcess to first process whose name is "Windsurf"
    set windowNames to {}
    repeat with w in windows of wsProcess
